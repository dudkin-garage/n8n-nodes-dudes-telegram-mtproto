...
function extractInviteHash(raw) {
    ...
    if (message.media instanceof telegram_1.Api.MessageMediaDocument) {
        ...
        if (document && Array.isArray(document.attributes)) {
            for (const attr of document.attributes) {
                if (attr instanceof telegram_1.Api.DocumentAttributeFilename) {
...
function getSenderId(message) {
    ...
    if (message.fromId.userId)
        return normalizeId(message.fromId.userId);
    if (message.fromId.channelId)
        return normalizeId(message.fromId.channelId);
    if (message.fromId.chatId)
        return normalizeId(message.fromId.chatId);
    return undefined;
}
    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            try {
                ...
                let entity;
                if (inviteHash) {
                    const result = await client.invoke(new telegram_1.Api.messages.ImportChatInvite({ hash: inviteHash }));
